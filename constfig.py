...
class _C(object):
    ...
    def load_config(self):
        try:
            ...
            logging.warning(
                "Configuration file config.yaml is missing. Using default values."
            )
        finally:
            self.set_constants()
    ...
    def validate_config(self):
        ...
        assert (
            len(self.LISTEN_IP.split(".")) == 4
        ), "LISTEN_IP has an unexpected number of octets"
        assert all(
            [ip.isnumeric() for ip in self.LISTEN_IP.split(".")]
        ), "LISTEN_IP is not a valid IP address."
        assert all(
            [0 <= int(ip) < 255 for ip in self.LISTEN_IP.split(".")]
        ), "LISTEN_IP is not a valid IP address."
        ...
        # Validate API_KEYS
        assert self._is_non_string_iterable(
            self.API_KEYS
        ), f"Expected list of API_KEYS, got {type(self.API_KEYS)}"
